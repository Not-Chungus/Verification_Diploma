...
always @(posedge F_if.clk or negedge F_if.rst_n) begin 
	if (!F_if.rst_n) begin
		wr_ptr <= 0; //BUG: wr_ack isn't reset correctly
	end				 //BUG: overflow, underflow not reset correctly
		...
		F_if.wr_ack <= 0; 
		if (F_if.full & F_if.wr_en) //BUG: overflow condition is wrong 
			F_if.overflow <= 1;
		else
			F_if.overflow <= 0;
	end
...
always @(posedge F_if.clk or negedge F_if.rst_n) begin
	if (!F_if.rst_n) begin //BUG: data out isn't reset correctly
		rd_ptr <= 0;
	end
...
always @(posedge F_if.clk or negedge F_if.rst_n) begin
	if (!F_if.rst_n) begin
		...
		if	( ({F_if.wr_en, F_if.rd_en} == 2'b10) && !F_if.full) 
			...
			count <= count - 1; //BUG: count isn't updated when both read and write are enabled
	end
end

assign F_if.full = (count == F_if.FIFO_DEPTH)? 1 : 0;
assign F_if.empty = (count == 0)? 1 : 0;
assign F_if.underflow = (F_if.empty && F_if.rd_en)? 1 : 0; //BUG: underflow is sequential
assign F_if.almostfull = (count == F_if.FIFO_DEPTH-2)? 1 : 0; //BUG : almostfull condition is wrong 
