...
//Reset sequence===========================================================
// Assert reset for two driven cycles, then deassert it with the FIFO idle.
// Every test starts this sequence before its scenario-specific sequence.
class FIFO_reset_seq extends uvm_sequence #(FIFO_seq_item);
  ...
  task body;
    repeat (2) begin
      seq_item = FIFO_seq_item::type_id::create("reset_assert_item");
      start_item(seq_item);
      seq_item.rst_n  = 0;
      seq_item.wr_en  = 0;
      seq_item.rd_en  = 0;
    ...
    seq_item = FIFO_seq_item::type_id::create("reset_release_item");
    start_item(seq_item);
    seq_item.rst_n  = 1;
    seq_item.wr_en  = 0;
    seq_item.rd_en  = 0;
