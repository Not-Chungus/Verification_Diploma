...
always @(posedge F_if.clk or negedge F_if.rst_n) begin 
	if (!F_if.rst_n) begin
		...
		F_if.wr_ack <= 0;
		wr_ptr <= wr_ptr; 
		if (F_if.wr_en && (count == FIFO_DEPTH) && !F_if.rd_en)
			F_if.overflow <= 1;
		else
			F_if.overflow <= 0;
	end
...
// ============================================================
  ...
  property p_almost_full;
      @(posedge F_if.clk) disable iff (!F_if.rst_n)
      (F_if.almostfull) |-> (count == (FIFO_DEPTH - 1));
  endproperty
...
if(!F_if.rst_n) begin
    ...
    a_count_reset: assert final(count == 0);
	end
end
